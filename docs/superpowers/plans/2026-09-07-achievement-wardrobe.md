...
- [ ] Write real temporary-store tests, including the boundary below; run and observe missing feature failures.
...
- [ ] Implement catalog, sanitized persistence with atomic replacement and retryable dirty state, initialization from history, cumulative days and settings normalization.
- [ ] Verify round-trip, wrong-slot normalization, invalid/malformed state, threshold boundaries, historical unlocks and write failure followed by successful retry. Commit owned files.
...
- [ ] Write and run renderer tests showing that an outfit changes face/head pixels and preserves paws, that distinct outfits produce distinct cached output and that None matches the old renderer.
...
- [ ] Draw editable original SVG accessories with matching bold rounded linework. Keep hats inside canvas, glasses aligned to eyes for every cat and the mouth visible. Avoid raster images or external SVG references.
- [ ] Add composition before rasterization with hat/glasses in cache keys; validate IDs and slot without a progression dependency. Preserve alpha behavior and empty outfit. Bundle assets and document editing.
- [ ] Render all six cats/four poses at 72, 120 and 210 pixels with accessories; verify transparent borders and stable upper-head alignment. Export a contact sheet in .debug and inspect it. Run focused existing artwork tests and commit only owned files.
...
- [ ] Write failing tests for locked choices, selecting None, real preview changes, saved shared outfit, cancellation, live unlocking, paused activity and load-frame outfit propagation.
...
- [ ] Build the wardrobe with thumbnails, one choice per slot, requirement/progress text, shared animated preview, scrollable layout and a compact unlock notice.
- [ ] Integrate persistence, enabled-key evaluation, historical startup rewards, safe optional tray notifications and all platform render paths. Retain existing settings save/cancel behavior.
- [ ] Run focused wardrobe/app tests; document rewards and preview/apply behavior. Commit owned files.

### Task 4: Review and delivery

- [ ] Run the full unittest suite from the feature worktree using the shared .venv Python.
- [ ] Inspect Wardrobe at normal and narrow sizes with temporary data, plus all accessory combinations in rendered artwork; verify changes do not clip or cover the mouth.
- [ ] Review spec compliance and code quality with a separate reviewer; resolve actionable findings.
- [ ] Build with PyInstaller from the feature worktree and inspect bundled accessory entries; do not publish a release.
- [ ] Record test and visual evidence, then integrate the verified feature into the original clean checkout and leave it ready to run.
